...
export default function Sidebar({ authEnabled = false }: SidebarProps) {
  ...
  return (
    <nav className="z-30 flex w-16 shrink-0 flex-col items-center border-r border-spectre-cyan/15 bg-black/40 py-4 backdrop-blur-xl">
      {/* Brand mark */}
      <Link
        href="/visor"
        ...
        {NAV.map(({ href, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              ...
              {/* hover tooltip */}
              <span className="pointer-events-none absolute left-12 z-50 whitespace-nowrap rounded-sm border border-spectre-cyan/20 bg-black/90 px-2 py-1 font-mono text-[10px] uppercase tracking-widest text-spectre-text opacity-0 backdrop-blur transition-opacity duration-150 group-hover:opacity-100">
                {label}
              </span>
            </Link>
          );
      ...
      {authEnabled && (
        ...
        >
          ...
          <span className="pointer-events-none absolute left-12 z-50 whitespace-nowrap rounded-sm border border-spectre-magenta/20 bg-black/90 px-2 py-1 font-mono text-[10px] uppercase tracking-widest text-spectre-text opacity-0 backdrop-blur transition-opacity duration-150 group-hover:opacity-100">
            Disconnetti
          </span>
        </button>
      )}
