// Finestre di invio e ritmo: fasce 9-13 / 16-20 Europe/Rome,
// niente weekend, delay random 60-240s tra messaggi.
...
/** true se ora è una finestra valida di invio (lun-ven, 9-13/16-20). */
export function inSendWindow(date = new Date()) {
  const { hour, weekday } = romeParts(date);
  if (weekday === "sab" || weekday === "dom") return false;
